...
export const Assessment = () => {
    ...
    const [multiAudioBlobs, setMultiAudioBlobs] = useState({});
    const [multiAudioURLs, setMultiAudioURLs] = useState({});
                ...
                if (activeSkillName) {
                    ...
                    if (category === 'Soft') {
                        ...
                        if (evalInfo.scenarios) {
                            ...
                            setSoftRecordings(new Array(evalInfo.scenarios.length).fill(null).map(() => ({
                                blob: null, url: null, isRecording: false, status: 'idle'
                            })));
                            setMediaRecorders(new Array(evalInfo.scenarios.length).fill(null));
                        }
    ...
    const submitTechnicalAssessment = async () => {
        ...
        try {
            ...
            formData.append('case_study_text', structuredCaseStudyText);
            if (caseStudyFile) {
    ...
    const resetEvaluation = () => {
        ...
        if (activeSkillName) {
            // Stay on the same skill but refresh questions
            const category = skillDetails?.category || 'Soft';
            const evalInfo = getShortEvaluationForSkill(activeSkillName, category);
            setEvaluationData(evalInfo);
            if (evalInfo.scenarios) {
                setSoftMultiAnswers(new Array(evalInfo.scenarios.length).fill(""));
            ...
            setSoftRecordings([]);
            setMediaRecorders([]);
            navigate('/assessment'); // Returns to hub
        }
    ...
    if (activeSkillName && !skillDetails) {
        return (
            <DashboardLayout user={user} onLogout={handleLogout}>
                <div style={{ textAlign: 'center', padding: '4rem 2rem', marginTop: '2rem' }}>
                    ...
                    <h2 style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>Failed to Load Skill Data</h2>
                    <p style={{ color: 'var(--color-text-muted)', marginBottom: '2rem' }}>
                        We couldn't load the evaluation data. Please ensure the backend server is running and try again.
                    </p>
                    <Button onClick={() => navigate('/assessment')}>Return to Hub</Button>
                </div>
            </DashboardLayout>
        );
    }
                            ...
                            <div style={{
                                                ...
                                                {scenarioModes[idx] === 'text' ? (
                                                    ...
                                                    <div style={{ background: 'var(--color-white)', padding: '1.25rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                                                        {softRecordings[idx].status === 'idle' && (
                                                            <Button
                                                                variant="primary"
                                                                onClick={() => startSoftRecording(idx)}
                                ...
                                <div style={{ marginTop: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    {submissionMode === 'quiz' && !quizLoading ? (
                                        ...
                                        <div style={{ fontSize: '1.1rem', color: 'var(--color-text-muted)', fontWeight: 600 }}>
                                            {softRecordings.filter(r => r.blob).length} / {evaluationData.scenarios.length} recorded
                                        </div>
                                    ) : <div />}
