...
def create_upskill_plan(req: UpskillPlanRequest):
    ...
    # 1. Identify weak skills (progress < 70)
    ...
    if not weak_skills:
        # If no weak skills, provide a general upskill plan based on their major
...
def get_upskill_plan_endpoint(email: str):
    ...
    if not plan:
        # If no plan exists, return empty or trigger a message
...
async def extract_skills_from_cv(file: UploadFile = File(...)):
    ...
    # If extraction fails or text is empty, return empty list (no crash)
    if not cv_text or not cv_text.strip():
        logger.warning(f"Empty text extracted from {file.filename}")
        return {"skills": []}
    
    # Clean text: remove non‑printable chars, normalize whitespace
    import re
    ...
    for skill in all_skills:
        ...
        # Match if skill name or any keyword appears in CV text
...
async def submit_major_assessment(
    ...
    # 6. Store Result History (WITHOUT modifying user's profile yet)
    result_doc = {
        "email": email,
        "major": major,
