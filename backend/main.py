...
async def sync_jobs():
    ...
    rapidapi_key = os.getenv("RAPIDAPI_KEY")
    if not rapidapi_key:
        raise HTTPException(status_code=500, detail="RAPIDAPI_KEY is not set in environment.")

    url = "https://jobs-api14.p.rapidapi.com/v2/bing/search"
    headers = {
        "X-RapidAPI-Key": rapidapi_key,
    ...
    params = {
        "datePosted": "week",
        "query": "IT or Software Developer or Computer Science",
        "remoteOnly": "false",
    ...
    try:
        ...
        api_jobs = res_data.get("data", [])
        
        sync_count = 0
        for job in api_jobs:
            job_title = job.get("title", "IT Specialist")
            company = job.get("company", "Tech Company")
            location = job.get("location", "Oman")
            post_url = job.get("postUrl", "https://bing.com/jobs")
            description = job.get("description", "")
            
            # Extract skills based on description and title keywords
            inferred_skills = []
            skill_dictionary = [
                "Python", "JavaScript", "React", "SQL", "Cyber Security", 
                "Cloud Computing", "Machine Learning", "Data Analysis", 
                "Networking", "Technical Support", "ERP Systems", "IT Project Management"
            ]
            for skill in skill_dictionary:
                if skill.lower() in description.lower() or skill.lower() in job_title.lower():
                    inferred_skills.append(skill)
            
            if not inferred_skills:
                inferred_skills = ["Technical Support", "Networking"]
            ...
            job_document = {
                ...
                "Industry": "Cloud Computing" if "cloud" in description.lower() else "Software Engineering",
                "Key_Skills": ", ".join(inferred_skills),
        ...
        return {
            "success": True,
            "message": f"Successfully synchronized {sync_count} jobs from Jobs API to MongoDB.",
            "source": "Jobs API (Bing Search)"
        }
    except Exception as e:
